...
if (! function_exists('normalizeSerializeArray')) {
     ...
     */
    function normalizeSerializeArray($queryDatas): array
    {
        ...
        foreach ($filters as $filter) {
            if (isset($finalFilters[$filter['name']])) {
                ...
                if (is_array($filter['value'])) {
                    $finalFilters[$filter['name']] = array_filter($filter['value']);
                } else {
...
if (! function_exists('logToFile')) {
     ...
     * @throws JsonException
     */
    function logToFile(string $channel, $api, $request, $response, array $times = [], string $level = 'info'): void
    {
        if (! $times) {
...
if (! function_exists('realFileSize')) {
     ...
     */
    function realFileSize(string $path)
    {
        if (! file_exists($path)) {
            return false;
        }
